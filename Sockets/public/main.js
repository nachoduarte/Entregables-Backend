...
function renderProductList(data) {
    if (data.length === 0)
        return (tbodyProducts.innerHTML = '<div class="alert alert-danger mt-4">No hay productos</div>')
...
function handleProductFormError(error) {
    if (error) {
        productFormError.innerHTML = `<div class="alert alert-danger"> ${error}</div>`
    } else {
...
chatForm.addEventListener('submit', (e) => {
    ...
    const newMessage = { email: email.value, message: message.value, date: Date.now() }
    socket.emit(keys.CHAT_ADD_MESSAGE, newMessage)
})

function handleChatFormError(error) {
    if (error) {
        chatFormError.innerHTML = `<div class="text-danger mt-1">${error}</div>`
    } else {
